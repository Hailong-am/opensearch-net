...
### Changed
- Changed the default serialization engine from the vendored Utf8Json to System.Text.Json. The legacy Utf8Json engine can be re-enabled via `.UseUtf8Json(true)` on `ConnectionSettings` or the `OSC_USE_UTF8JSON=true` environment variable. Utf8Json support will be removed in 3.0.0 ([#388](https://github.com/opensearch-project/opensearch-net/issues/388)).
